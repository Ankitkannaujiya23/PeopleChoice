...
export const userSignUp = async (url, model) => {
    let data;
    await axios.post(url, model).then(res => {
        data = res.data;
...
export const userLogin = async (url, model) => {
    let data;
    await axios.post(`${baseURL}/${url}`, model).then((res) => {
        data = res.data;
...
export const createPost = async (url, model, token, id) => {
    ...
    await axios.post(`${baseURL}/${url}`, model, { headers: { token: token } }).then((res) => {
        data = res.data;
...
export const updatePost = async (url, model, token, id) => {
    let data = '';
    //for post api calls we have to pass the body after just url and header after the body like ==> url,body,{headers}
    await axios.put(`${baseURL}/${url}`, model, { headers: { token: token } }).then((res) => {
        data = res.data;
...
export const getSinglePost = async (url) => {
    let token = getAccessToken();
    let data;
    await axios.get(`${baseURL}/${url}`, { headers: { token: token } }).then(res => {
        if (res.status === 200) {
...
export const deleteBlogPost = async (url) => {
    let data;
    await axios.delete(`${baseURL}/${url}`, { headers: { token: token } }).then((res) => {
        data = res.data;
    }).catch(error => console.log(error));
    return data;
}
...
export const fetchAllComments = async (url) => {
    ...
    let data;
    await axios.get(`${baseURL}/${url}`, { headers: { token } }).then((res) => {
        data = res.data;
    }).catch((error) => console.log(error))
    return data;
}
