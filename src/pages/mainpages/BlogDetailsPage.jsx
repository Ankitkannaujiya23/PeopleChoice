...
import { deleteBlogPost, getSinglePost } from '../../services/apiCalls';
...
const BlogDetailsPage = (props) => {
    ...
    useEffect(() => {
        ...
        const getBlogPostById = async () => {
            let response = await getSinglePost(`blogPostController/fetchSinglePost/${id}`);
    ...
    const deletePost = async () => {
        let response = await deleteBlogPost(`blogPostController/deletePost/${Data._id}`);
                            ...
                            <div className="lg:px-16  mx-auto grid grid-cols-12 space-x-3 gap-2">
                                <div className=" lg:col-span-8 md:col-span-8 col-span-12">
                                    <div className="rounded-lg  overflow-hidden">
                                        <img alt="content" className="object-cover object-center h-full w-full" src={Data.image ? Data.image : blogImg} />
                                    </div>
                                    <div className="   border-gray-800 sm:border-t-0 border-t mt-8 lg:mt-4 md:mt-4   text-center sm:text-left">
                                        <div className="category flex items-center justify-between w-full">
                                            <div>
                                                <span className='text-xl mb-2 flex items-center space-x-2 text-orange-500'><FcTimeline /><span>{Data.category}</span></span>
                                            </div>
                                            {UserDetails.name === Data.username && <div className="actionBar flex space-x-2 text-lg">
                                                <Link to={`/updateBlog/${Data._id}`} state={{ post: Data }}><FaRegEdit className='text-green-600 cursor-pointer' /></Link>
                                                <RiDeleteBin6Line className='text-red-500 cursor-pointer' onClick={deletePost} />
                                            </div>}
                                        </div>
                                        <p className='text-3xl mb-2 text-[#262566] font-semibold '>{Data.title}</p>
                                        <p className="leading-relaxed text-lg mb-4 text-[#6e6e6e]">{Data.description}</p>
                                    </div>
                                ...
                                <div className="lg:col-span-4 col-span-12 space-y-6">
                                    <div className="search-sec shadow-md">
                                        <p className=' bg-gray-200 border-l-4 border-[#f53855] py-2 text-lg text-black px-2 font-semibold'> Search Now</p>
                                        <div className="p-2">
                                            <SearchBar />
                                        </div>
                                    </div>
                                    ...
                                    <div className="category-sec shadow-md">
                                        <p className=' bg-gray-200 border-l-4 border-[#f53855] py-2 text-lg text-black px-2 font-semibold'> Popular Post</p>
                                        <div className='p-2'>
                                            <PopularPostCard />
                                            <PopularPostCard />
                                            <PopularPostCard />
